...
//	result = engine.RegisterObjectMethod("Quat", "Quat@ Normalized() const", asMETHOD(Quat, Normalized), asCALL_THISCALL);																																							ASSERT(result >= asSUCCESS);
	...
	result = engine.RegisterGlobalFunction("Quat@ Slerp(const Quat& in, const Quat& in, float)", asFUNCTION((AngelScriptGenericAPI<Quat>::StaticFunc<const Quat&, const Quat&, float, &Quat::Slerp>)), asCALL_GENERIC);																ASSERT(result >= asSUCCESS);
