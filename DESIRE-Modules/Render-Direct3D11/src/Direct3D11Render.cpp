...
void Direct3D11Render::SetBlendModeSeparated(EBlend srcBlendRGB, EBlend destBlendRGB, EBlendOp blendOpRGB, EBlend srcBlendAlpha, EBlend destBlendAlpha, EBlendOp blendOpAlpha)
{
	...
	constexpr D3D11_BLEND blendConversionTable[] =
	{
	...
	constexpr D3D11_BLEND_OP equationConversionTable[] =
	{
...
void Direct3D11Render::CreateBackBuffer(uint32_t width, uint32_t height)
{
	...
	DX_CHECK_HRESULT(hr);
	hr = d3dDevice->CreateRenderTargetView(backBufferTexture, nullptr, &pBackBufferRenderTargetView);
	DX_CHECK_HRESULT(hr);
	...
	hr = d3dDevice->CreateDepthStencilView(depthStencilTexture, nullptr, &pBackBufferDepthStencilView);
	DX_CHECK_HRESULT(hr);
...
void Direct3D11Render::SetTexture(uint8_t samplerIdx, const Texture& texture, EFilterMode filterMode, EAddressMode addressMode)
{
	...
	constexpr D3D11_TEXTURE_ADDRESS_MODE addressModeConversionTable[] =
	{
...
void Direct3D11Render::SetInputLayout()
{
	...
	if(pActiveMesh != nullptr)
	{
		...
		{
			constexpr D3D11_INPUT_ELEMENT_DESC s_attribConversionTable[] =
			{
			...
			constexpr DXGI_FORMAT s_attribTypeConversionTable[][4] =
			{
