...
#define _SINCOS_KC2  7.54978995489e-8f
	...
	res = _mm_blendv_ps(cx, sx, _mm_cmpeq_ps(_mm_and_ps(offset, toM128(0x1)), _mm_setzero_ps()));
...
static DESIRE_FORCE_INLINE void sincosf4(__m128 x, __m128 *s, __m128 *c)
{
	...
	// Find the quadrant the angle falls in
	// using:  q = (int)(ceil(abs(xl)) * sign(xl))
//	const __m128i q = _mm_cvtps_epi32(_mm_add_ps(xl, _mm_blendv_ps(_mm_set1_ps(0.5f), xl, 0x80000000)));
	const __m128i q = _mm_cvtps_epi32(xl);
	...
	*s = _mm_blendv_ps(cx, sx, sinMask);
	*c = _mm_blendv_ps(cx, sx, cosMask);
	...
	*s = _mm_blendv_ps(_mm_xor_ps(toM128(0x80000000), *s), *s, sinMask);
	*c = _mm_blendv_ps(_mm_xor_ps(toM128(0x80000000), *c), *c, cosMask);
}
...
public:
	...
	static DESIRE_FORCE_INLINE void SetXYZ(__m128& vec, __m128 xyz)
	{
		...
		vec = _mm_blendv_ps(xyz, vec, mask_w);
	}
