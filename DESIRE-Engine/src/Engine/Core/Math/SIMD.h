...
public:
	...
	// Construct by setting all elements to zero value
	static inline simd128_t ConstructZero()
	{
#if DESIRE_USE_SSE
...
#else
		return SIMD::Construct(0.0f);
#endif
	}

	// Load x, y, z, and w elements from the first four elements of a float array
	static inline simd128_t LoadXYZW(const float* pValues)
	{
#if DESIRE_USE_SSE
		return _mm_loadu_ps(pValues);
#elif DESIRE_USE_NEON
		return vld1q_f32(pValues);
#else
		return SIMD::Construct(pValues[0], pValues[1], pValues[2], pValues[3]);
#endif
	}

	// Store x, y, z, and w elements in the first four elements of a float array
	static inline void StoreXYZW(simd128_t vec, float* pValues)
	{
#if DESIRE_USE_SSE
		_mm_storeu_ps(pValues, vec);
#elif DESIRE_USE_NEON
		vst1q_f32(pValues, vec);
#else
		memcpy(pValues, vec.f32, 4 * sizeof(float));
#endif
	}
...
{
	return SIMD::Construct(*reinterpret_cast<const float*>(&x));
}
...
//   sx = xl + xl3 * ((S0 * xl2 + S1) * xl2 + S2)
	...
	__m128 sinMask = _mm_cmpeq_ps(_mm_and_ps(offsetSin, toM128(0x1)), SIMD::ConstructZero());
	__m128 cosMask = _mm_cmpeq_ps(_mm_and_ps(offsetCos, toM128(0x1)), SIMD::ConstructZero());
	*s = SIMD::Blend(cx, sx, sinMask);
	*c = SIMD::Blend(cx, sx, cosMask);

	// Flip the sign of the result when (offset mod 4) = 1 or 2
	sinMask = _mm_cmpeq_ps(_mm_and_ps(offsetSin, toM128(0x2)), SIMD::ConstructZero());
	cosMask = _mm_cmpeq_ps(_mm_and_ps(offsetCos, toM128(0x2)), SIMD::ConstructZero());
